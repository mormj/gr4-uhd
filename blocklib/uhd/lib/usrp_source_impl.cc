...
usrp_source_impl::usrp_source_impl(const ::uhd::device_addr_t& device_addr,
    ...
    : usrp_block("usrp_source"),
      ...
      _overflow_count(0),
      _overflow_log_interval(gr::prefs::get_long("uhd", "logging_interval_ms", 750))
{
    // stream_args determine the ports
    if (stream_args.cpu_format == "fc32") {
        add_port(port<gr_complex>::make("out0", port_direction_t::OUTPUT));
        if (!stream_args.channels.empty()) {
            for (size_t ch = 1; ch < stream_args.channels.size(); ch++) {
                add_port(port<gr_complex>::make(std::string("out") + std::to_string(ch),
...
void usrp_source_impl::set_gain(double gain, size_t chan, const std::string& direction)
{
    chan = _stream_args.channels[chan];
    if (direction == direction_tx()) {
...
void usrp_source_impl::_cmd_handler_tag(const pmtf::pmt& tag) { _tag_now = true; }
...
work_return_t usrp_source_impl::work(work_io& wio)
{
    ...
    case ::uhd::rx_metadata_t::ERROR_CODE_NONE:
        if (_tag_now) {
            ...
            const pmtf::pmt val =
                pmtf::vector<pmtf::pmt>{ _metadata.time_spec.get_full_secs(),
                              _metadata.time_spec.get_frac_secs() };
            // create a tag set for each channel
            auto nw = wio.outputs()[0].nitems_written();
            for (size_t i = 0; i < _nchan; i++) {
                wio.outputs()[i].add_tag(
                    nw, pmtf::map{ { TIME_KEY, val }, { "srcid", _id } });
                wio.outputs()[i].add_tag(
                    nw,
                    pmtf::map{ { RATE_KEY, pmtf::scalar<double>(_samp_rate) },
                               { "srcid", _id } });
                wio.outputs()[i].add_tag(
                    nw,
                    pmtf::map{
                        { FREQ_KEY, pmtf::scalar<double>(this->get_center_freq(i)) },
                        { "srcid", _id } });
            }
        }
        break;
